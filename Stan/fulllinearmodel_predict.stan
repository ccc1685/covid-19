...
functions { // time transition functions for beta and sigmac
        ...
        data {
          ...
          int<lower=1> n_predict;     // number of time points to predict
          int tp[n_predict]; // time points to predict calibrate to t0 of data 
          }
        ...
        generated quantities {
            ...
            real llx[n_obs, 3];
            real y_predict[n_predict,3]; //predict C, R, D
            ...
            //print(u_init)
             ...
             Rt[1] = betat*(sigma+q*sigmact)/sigma/(sigmact+sigmau);
            ...
            for (i in 1:n_obs) {
             ...
             real theta[10] = {f1,f2,sigmar,sigmad,sigmau,q,mbase,mlocation,cbase,clocation};
             u = integrate_ode_rk45(SIR, u_init, t0, tp, theta, x_r, x_i,1e-3,1e-3,2000);
