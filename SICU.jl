...
"""
function mcmc(data::DataFrame,cols::Array,p::Array,N::Float64,total::Int,sicrfn!,temp = 1.)
...
function ll(data,cols,p,N,sicrfn!)
	...
	uin = initprep(data,cols,firstday)/N
	prediction,days = model(p,uin,float.(tspan),N,sicrfn!)
	chi = 0
	for set in length(cols)
...
function initprep(data,cols,firstday)
	uin = Array{Float64,1}(undef,length(cols))
	for i in eachindex(cols)
		uin[i] = float(data[firstday,cols[i]])
	end
	return uin
end
...
function model(pin,uin,tspan,N,sicrfn!)
	...
	return sol[1:length(uin),:]*N, round.(Int,sol.t)
end
